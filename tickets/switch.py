...
class Connect:
	def __init__(self, name: str, ip: str, vendor: str, model: str):
		self.session = None
		...
		self.switch = None
		allowed_models = [
			'Catalyst 6506', 'Catalyst 6513', 'Catalyst 6509-E', 'Catalyst 6509', 'S2995G-24FX',
			'S2995G-24FX-UPS', 'S2995G-24FX-DC', 'S2995G-48FX', 'S2990G-24FX', 'S3650G-48S'
		]
		if vendor == 'Cisco' and model in allowed_models:
			self.switch = Cisco()
		elif vendor == 'SNR' and model in allowed_models:
			self.switch = Snr()
		if not self.switch:
			raise SwitchException(f"Обнаружена некорректная модель {model}")
