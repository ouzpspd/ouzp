...
class KtcMount:
    ...
    def fill_exist_line_from_kad(self):
        ...
        strs = [
            '- Использовать существующую %тип линии связи% от %узел связи% до клиента.',
...
class OpticKtcMount(KtcMount):
    ...
    def fill_vars(self):
        ...
        if self.change_log == 'меняется':
            ...
            if self.params.get('device_client'):
                strs.append('- На стороне клиента %вид работ% [%установленный тип конвертера/передатчика на стороне клиента% на ]%тип конвертера/передатчика на стороне клиента%')
...
class PassageCsw(Csw):
    ...
    def fill_vars(self):
        """Метод, заполняющий шаблон ТР переменными."""
        super().fill_vars()
        if not (self.change_log == 'меняется' and self.change_physic == 'не меняется' and self.exist_mount_type == self.mount_type):
