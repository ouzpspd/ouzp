...
def _get_pm_vars(value_vars):
    ...
    if value_vars.get('spd') == 'ППМ':
        ...
        if value_vars.get('rtk_form').get('type_pm') == 'ПМ':
            ...
            add_hidden_vars[
                '- Добавить на ресурсе в ИС Cordis информацию: "B2B FVNO РТ. Заявка СПП: %№ заявка СПП%".'] = \
                '- Добавить на ресурсе в ИС Cordis информацию: "B2B FVNO РТ. Заявка СПП: %№ заявка СПП%".'
...
-- Сервис авторизации Хот-Спот."""
                if value_vars.get('hotspot_local_wifi') is True and logic_csw:
                    stroka = templates.get("Организация услуги Хот-Спот Премиум c локальной сетью WiFi для сотрудников компании.")
                else:
                    if value_vars.get('type_hotspot') == 'Хот-Спот Премиум':
    ...
    if counter_line_services > 0:
        ...
        if logic_csw == True:
            ...
            if logic_csw_1000 == True:
                static_vars['пропускная способность'] = '1000'
            else:
                static_vars['пропускная способность'] = '100'
            templates = value_vars.get('templates')
...
def _passage_services(result_services, value_vars):
    ...
    if value_vars.get('type_passage') == 'Перенос сервиса в новую точку' or value_vars.get('type_passage') == 'Перенос точки подключения':
        stroka = templates.get("Перенос ^сервиса^ %название сервиса% в новую точку подключения")
        if value_vars.get('type_passage') == 'Перенос точки подключения':
...
def get_add_kad(value_vars):
    ...
    if value_vars.get('type_add_kad') == 'Установка 2-го медного кад в гирлянду':
        ...
        static_vars['название коммутатора нижестоящий'] = value_vars.get('deleted_kad')
        ...
        if value_vars.get('delete_kad'):
            hidden_vars[
                '. Второй в гирлянде КАД %название коммутатора нижестоящий% демонтировать'
            ] = '. Второй в гирлянде КАД %название коммутатора нижестоящий% демонтировать'
            hidden_vars[
                '- Демонтировать высвобожденный коммутатор %старая модель коммутатора% %название коммутатора нижестоящий% вместе со «стековым» линком.'
            ] = '- Демонтировать высвобожденный коммутатор %старая модель коммутатора% %название коммутатора нижестоящий% вместе со «стековым» линком.'
            hidden_vars[
...
доставить на склад."""
            hidden_vars[
                '- Высвобожденный коммутатор %название коммутатора нижестоящий% вместе со «стековым» линком демонтировать в' +
                ' ИС Cordis и разобрать настройки на оставшемся КАД на порту подключения демонтированного КАД.'
                ] = '- Высвобожденный коммутатор %название коммутатора нижестоящий% вместе со «стековым» линком демонтировать в' + \
                    ' ИС Cordis и разобрать настройки на оставшемся КАД на порту подключения демонтированного КАД.'

        else:
            hidden_vars['и %название коммутатора нижестоящий% '] = 'и %название коммутатора нижестоящий% '
            hidden_vars[
                '- Установить в свободный гигабитный порт КАД %название коммутатора нижестоящий% оптический передатчик SFP WDM, дальность до 3км (6dB), 1550нм.'
            ] = '- Установить в свободный гигабитный порт КАД %название коммутатора нижестоящий% оптический передатчик SFP WDM, дальность до 3км (6dB), 1550нм.'
            hidden_vars[' - 2 шт'] = ' - 2 шт'
            hidden_vars[
                '- Скроссировать порт дополнительного КАД с портом КАД %название коммутатора нижестоящий%. Использовать оптический патчкорд SM SC-SC 1m.'
            ] = '- Скроссировать порт дополнительного КАД с портом КАД %название коммутатора нижестоящий%. Использовать оптический патчкорд SM SC-SC 1m.'
            hidden_vars[
                '- Запустить существующий КАД %название коммутатора нижестоящий% от оптогигабитного коммутатора на магистрали 1 Гбит/с.' +
