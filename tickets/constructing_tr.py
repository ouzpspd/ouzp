            ...
            if service.endswith('|'):
                if value_vars.get('type_phone') == 'st':
                    ...
                    if vgw in ['D-Link DVG-5402SP', 'Eltex RG-1404G или Eltex TAU-4M.IP', 'Eltex TAU-8.IP']:
                        static_vars['магистральный порт тел. шлюза'] = 'WAN порт'
                    else:
                        static_vars['магистральный порт тел. шлюза'] = 'Ethernet Порт 0'
            ...
            if logic_csw_1000 or value_vars.get('logic_change_gi_csw'):
                ...
                if old_kad.split('-')[1] != kad.split('-')[1]:
                    ...
                    if vgws and len(vgws) == 1:
                        ...
                        hidden_vars[
                            ' и тел. шлюза %указать название тел шлюза%'] = ' и тел. шлюза %указать название тел шлюза%'
                        hidden_vars[
                            ' и тел. шлюз %указать название тел шлюза%'] = ' и тел. шлюз %указать название тел шлюза%'
                        ...
                        hidden_vars[
                            '- Перед проведением работ запросить ОНИТС СПД сменить реквизиты тел. шлюза %указать название тел шлюза% на ZIP.'] = '- Перед проведением работ запросить ОНИТС СПД сменить реквизиты тел. шлюза %указать название тел шлюза% на ZIP.'
                        hidden_vars[
                            ' и тел. шлюза %указать название тел шлюза%'] = ' и тел. шлюзов %указать название тел шлюза%'
                        hidden_vars[
                            ' и тел. шлюз %указать название тел шлюза%'] = ' и тел. шлюзы %указать название тел шлюза%'
    ...
    if value_vars.get('type_passage') == 'Перенос сервиса в новую точку' or value_vars.get('type_passage') == 'Перенос точки подключения':
        ...
        if sreda == '1':
            ...
            if value_vars.get('exist_sreda') != '2':
                hidden_vars['- На стороне клиента %установить/заменить% [%указать существующий конвертер/передатчик на стороне клиента% на ]%тип конвертера/передатчика на стороне клиента%'] = '- На стороне клиента %установить/заменить% [%указать существующий конвертер/передатчик на стороне клиента% на ]%тип конвертера/передатчика на стороне клиента%'
        ...
        static_vars['режим работы порта доступа'] = value_vars.get('speed_port')
        hidden_vars[
            '- На стороне клиента %установить/заменить% [%указать существующий конвертер/передатчик на стороне клиента% на ]%тип конвертера/передатчика на стороне клиента%'] = '- На стороне клиента %установить/заменить% [%указать существующий конвертер/передатчик на стороне клиента% на ]%тип конвертера/передатчика на стороне клиента%'
...
def passage_video(value_vars):
    ...
    if value_vars.get('pass_video_form').get('poe') == 'Сущ. POE-инжектор':
        ...
        hidden_vars[
            '- Установить в помещении клиента POE-коммутатор %POE-коммутатор%. Организовать линию от маршрутизатора клиента до POE-коммутатора. Включить организованную линию связи:'
        ] = '- Установить в помещении клиента POE-коммутатор %POE-коммутатор%. Организовать линию от маршрутизатора клиента до POE-коммутатора. Включить организованную линию связи:'
        hidden_vars['-- В свободный порт маршрутизатора;'] = '-- В свободный порт маршрутизатора;'
        hidden_vars[
            '-- В порт %указать номер порта POE-коммутатора% POE-коммутатора'
        ] = '-- В порт %указать номер порта POE-коммутатора% POE-коммутатора'
        if count_cameras < 5:
            static_vars['POE-коммутатор'] = 'D-Link DES-1005P'
            static_vars['указать номер порта POE-коммутатора'] = '5'
        else:
            static_vars['POE-коммутатор'] = 'Atis PoE-1010-8P'
            static_vars['указать номер порта POE-коммутатора'] = '10'
